...
app.main = {
    ...
    formValidate: function () {
        ...
        $('.request-callback-form').validate({
            rules: {
                phone: {
                    required: true
                },
            ...
            submitHandler: function (form) {
                var $form = $(form),
                    $formMain = $form.find('.form-main'),
                    $btn = $form.find('button[type="submit"]'),
                    $preloader = $form.find('.modal-preloader'),
                    $messageSuccess = $form.find('.form-message.success'),
                    $messageError = $form.find('.form-message.error');
                $btn.css('opacity', 0);
                $preloader.fadeIn();
                $messageSuccess.hide();
                $messageError.hide();
...
app.modals = {
    init: function () {
        this.callbackModalOpen();
        this.callbackModalClose();
        $(document).click(function(event) {
            if ($(event.target).closest('.callback-modal').length || $(event.target).closest('.jsRequestCallbackBtn').length) return;
            var $modal = $('.modal'),
                $formMain = $modal.find('.form-main'),
                $formMessage = $modal.find('.form-message');
    ...
    callbackModalOpen: function () {
        $('.jsRequestCallbackBtn').on('click', function (e) {
            e.preventDefault();
            var $this = $(this),
                $callbackModal = $this.siblings('.callback-modal');
            $callbackModal.slideDown();
        });
    },
    callbackModalClose: function () {
        $('.jsModalClose').on('click', function (e) {
            e.preventDefault();
            var $this = $(this);
